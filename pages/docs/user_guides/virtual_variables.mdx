...
<Callout type="tip" title="Note">
    Virtual Variables are currently only supported for Dynamic Interaction configurations. Support for static models will be added in the 0.9.6.1 release.
</Callout>
...
<Callout type="tip" title="Note">
    Prior to version 0.9.4.3 `getPrePredict` is not passed the Mongo Connection (`mongoClient`). In this case you will need to open a close a Mongo connection in the pre scoring logix, as shown in the code below. Note that this will increase the load on the database.
