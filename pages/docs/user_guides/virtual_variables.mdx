...
To use Virtual Variables for the contextual variables in a Dynamic Interaction configuration, the `getDynamicSettings` and `getPrepopulateContextualVariables` methods need to be called in the pre scoring logic. The template for doing so is the `PreScoreDynamic` class which is shown below. This will result in the values of the virtual variables being  added to the contextual variables objects in the `params` JSONObject that is passed through the runtime.
```java
