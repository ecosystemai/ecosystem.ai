# iOS App Framework

## Overview

The iOS UI for the Spend Personality module is designed to provide a user-friendly interface for interacting with the ecosystem.Ai platform. It allows users to input their customer number and receive personalized messages based on their spending personality.

## How it Works

Clone the provided repo and follow the instructions below to run the iOS UI for the Spend Personality module.

![image](/images/modules/ios.png)

Once the repo is cloned, read the README file for detailed instructions on how to run the iOS UI.
```bash
git clone https://github.com/ecosystemai/message-app-ios.git
```
